...
"use client";
import { MessageSquare, ChartNoAxesCombined } from "lucide-react";
...
type Props = { docId: string; mode: RightPaneMode; onModeChange: (mode: RightPaneMode) => void; pageIndex: number; selectionRequest?: SelectionRequest | null; visualRequest?: VisualRequest | null; ready: boolean; captureRequests?: CaptureRequest[]; onCapturesConsumed?: (ids: string[]) => void };
export default function RightPane({ docId, mode, onModeChange, pageIndex, selectionRequest, visualRequest, ready, captureRequests, onCapturesConsumed }: Props) {
  return <div className="flex h-full flex-col bg-[var(--surface-raised)]">
    <header className="flex shrink-0 items-center gap-2 border-b border-[var(--border-subtle)] px-4 py-2">
      ...
      <p className="ml-auto text-[11px] text-[var(--ink-500)]">Sur votre demande</p>
    </header>
    {!ready ? <div className="flex flex-1 items-center justify-center p-8 text-center text-sm text-[var(--ink-500)]">La préparation initiale donne au chat le contexte de toutes les pages. Vous pouvez déjà parcourir le PDF.</div> : <>
      <div className={`min-h-0 flex-1 ${mode !== "chat" ? "hidden" : ""}`}><ChatView key={docId} docId={docId} pageIndex={pageIndex} selectionRequest={selectionRequest} captureRequests={captureRequests} onCapturesConsumed={onCapturesConsumed} /></div>
