...
import ChatView, { type SelectionRequest } from "./ChatView";
import ManualVisual, { type VisualRequest } from "./ManualVisual";
export type RightPaneMode = "chat" | "visualizer";
type Props = { docId: string; mode: RightPaneMode; onModeChange: (mode: RightPaneMode) => void; pageIndex: number; selectionRequest?: SelectionRequest | null; visualRequest?: VisualRequest | null; ready: boolean };
export default function RightPane({ docId, mode, onModeChange, pageIndex, selectionRequest, visualRequest, ready }: Props) {
  return <div className="flex h-full flex-col bg-[var(--surface-raised)]">
    ...
    {!ready ? <div className="flex flex-1 items-center justify-center p-8 text-center text-sm text-[var(--ink-500)]">La préparation initiale donne au chat le contexte de toutes les pages. Vous pouvez déjà parcourir le PDF.</div> : <>
      <div className={`min-h-0 flex-1 ${mode !== "chat" ? "hidden" : ""}`}><ChatView docId={docId} pageIndex={pageIndex} selectionRequest={selectionRequest} /></div>
